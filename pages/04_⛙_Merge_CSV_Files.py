...
if uploaded_files is not None:
    ...
    if merged_csv:
        ...
        new_df = pd.DataFrame(generate_csv.check_iqr_data(extra_list), columns = ["fish", "location", "weight(kg)", "length(cm)", "depth(cm)", "weight diff(combined iqr)", "length diff(combined iqr)", "depth diff(combined iqr)"])
